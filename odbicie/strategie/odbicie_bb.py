...
def generate_odbicie_bb_entries(
    ...
    Jeśli enter_on_close=True, wchodzimy natychmiast na zamknięciu pierwszego
    ...
    Args:
        ...
        enter_on_close: Jeśli True, wchodzi natychmiast na zamknięciu pierwszego bara po sygnale.
    ...
    for _, sig in signals_df.iterrows():
        ...
        for i, (ts, row) in enumerate(future_df.iterrows()):
            ...
            if row['Low'] <= lower_band:
                # If enter_on_close: fill at bar's close; else at BB lower or gap-down open
                entry_price = row['Close'] if enter_on_close else min(row['Open'], lower_band)
