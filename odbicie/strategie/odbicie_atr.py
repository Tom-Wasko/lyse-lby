...
def generate_odbicie_atr_entries(
    ...
    Args:
        ...
        enter_on_close: Jeśli True, wchodzi natychmiast po zamknięciu pierwszego bara
    ...
    for _, sig in signals_df.iterrows():
        ...
        # Wyciągamy w przód max_setup_hold_bars świec
        ...
        for i, (ts, row) in enumerate(future_df.iterrows()):
            if row['Low'] <= target_entry_price:
                # If enter_on_close: fill at bar's close; else at target or gap-down open
                entry_price = row['Close'] if enter_on_close else min(row['Open'], target_entry_price)
